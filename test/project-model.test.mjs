...
test('legacy snapshots migrate and current metadata is preserved',()=>{
  ...
  assert.equal(current.algorithmVersion,'ep-lex-portfolio-2026.10.17');assert.equal(current.resultSummary.loaded,36);assert.equal(current.resultSummary.totalWeight,6692);
});
