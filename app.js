...
function enhanceSafetySlider(){
  ...
  slider.onchange=()=>{if(select.value==='secure'&&!ctuNoticeShown){ctuNoticeShown=true;showAppMessage(CTU_NOTICE,{title:'CTU 기준 적용',tone:'info'})}};
...
function renderResultSummary(){
  ...
  const rearrange=plan.reviews.filter(r=>r.severity==='rearrange').length,faces=plan.reviews.filter(r=>r.ctuFace).length,review=plan.reviews.length-rearrange-faces,restraint=ctuSecuringDirections(plan.ctu).length;
  const checks=[rearrange?`배치 재검토 ${rearrange}건`:'',faces?`CTU 열린 면 보강 ${faces}곳`:'',review?`현장 고정 검토 ${review}건`:'',restraint?`CTU 고정 필요 ${restraint}방향`:''].filter(Boolean);
...
async function exportPdf(){if(resultStale)return showAppMessage('입력이 바뀌었습니다. 다시 계산한 뒤 내보내 주세요.',{title:'결과가 최신이 아닙니다',tone:'warning'});
  if(!shipment)return;if(shipment.unallocated.length){showAppMessage(`미배치 화물 ${shipment.unallocated.length}개를 해결한 후 작업지시서를 만들 수 있습니다.`,{title:'PDF 작업지시서가 제한됩니다',tone:'warning'});return}if(!await confirmExportNotice())return;const printWindow=window.open('','loadwise-work-instruction');if(!printWindow){showAppMessage('작업지시서 새 창을 열 수 없습니다. 브라우저의 팝업 차단을 해제해 주세요.',{title:'새 창이 차단되었습니다',tone:'warning'});return}printWindow.document.write('<!doctype html><title>작업지시서 준비 중</title><p style="font-family:sans-serif;padding:24px">작업지시서를 준비하고 있습니다…</p>');
...
async function confirmExportNotice(){if(exportNoticeAccepted)return true;const ok=await showAppMessage(EXPORT_NOTICE,{title:'현장 확인이 필요한 검토용 문서입니다',tone:'warning',confirmAction:true,actionLabel:'확인하고 내보내기',cancelLabel:'취소'});if(ok===true)exportNoticeAccepted=true;return ok===true}
const EXPORT_NOTICE='이 계획은 작업 검토용 베타 결과입니다. 화물 강도·상부 허용하중, 축하중·바닥 집중하중, 마찰·동하중과 래싱 용량, 위험물 규정은 계산하지 않습니다. 실제 적재 전 담당 전문가가 현장 확인해야 합니다.';
async function exportPlan(){if(resultStale)return showAppMessage('입력이 바뀌었습니다. 다시 계산한 뒤 내보내 주세요.',{title:'결과가 최신이 아닙니다',tone:'warning'});if(!await confirmExportNotice())return;await loadXLSX();
  if(!result)return;
  ...
  loads.forEach((load,i)=>{
    ...
    const fixed=[...(load.securing?.dunnage||[]),...(load.securing?.airbags||[])];fixed.forEach((f,n)=>securing.push({'컨테이너 번호':i+1,'번호':n+1,'구분':f.type==='dunnage'?(f.kind==='beam'?'가로 각재':'부목'):'에어백','추천 위치':f.location||'화물 간극','관련 제품':f.product||'','X(mm)':Math.round(f.x),'Y(mm)':Math.round(f.y),'Z(mm)':Math.round(f.z),'길이(mm)':Math.round(f.l),'너비(mm)':Math.round(f.w),'높이(mm)':Math.round(f.h)}));(load.securing?.reviews||[]).forEach((review,n)=>securing.push({'컨테이너 번호':i+1,'번호':fixed.length+n+1,'구분':reviewLabel(review),'추천 위치':review.location,'관련 제품':review.product,'미지지 방향':review.axes||''}));const ctuReview=load.securing?.ctu,base=fixed.length+(load.securing?.reviews||[]).length;ctuSecuringDirections(ctuReview).map(d=>({'구분':'CTU 참고 계산','추천 위치':d.label,'관련 제품':d.worst?.product||'','필요 억제력(kN)':Number(d.forceKN.toFixed(1)),'전도 위험 화물':d.tipping,'막히지 않은 화물':d.unblocked,'가장 불리한 높이 비율':d.worst?Number(d.worst.ratio.toFixed(2)):'','전도 한계':d.worst?Number(d.worst.limit.toFixed(2)):''})).forEach((row,n)=>securing.push({'컨테이너 번호':i+1,'번호':base+n+1,...row}));
  });
