...
function runPackingEngine(input,onProgress=()=>{}){
  ...
  if(!engineWorker&&typeof Worker!=='undefined'&&location.protocol!=='file:')try{engineWorker=new Worker('engine-worker.js?v=20260925-11')}catch{engineWorker=null}
...
function buildSecuringPlan(load,transportMode=currentTransportMode()){
  ...
  if(recessed.length){
    ...
    recessed.filter(p=>p.z===0).forEach(p=>{
    ...
    const upper=recessed.filter(p=>p.z>0);
    if(upper.length){
      ...
      const fenceDepth=depth>=20?depth:FENCE_DEPTH,bottom=Math.min(...upper.map(p=>p.z))+50,top=Math.min(c.h-80,Math.max(...upper.map(p=>p.z+p.h))),levels=Math.max(2,Math.ceil((top-bottom)/550)+1);
  ...
  if(floorItems.length){const front=Math.min(...load.placed.map(p=>p.x)),upper=load.placed.filter(p=>p.z>0&&p.x<=front+300);if(upper.length){const top=Math.max(...upper.map(p=>p.z+p.h)),z=Math.max(...upper.map(p=>p.z))+Math.min(...upper.map(p=>p.h))*.5,strap={type:'dunnage',kind:'strap',axis:'y',side:'min',x:Math.max(0,front-22),y:0,z:Math.min(top-60,z),l:18,w:c.w,h:50,product:'문쪽 윗단 화물',location:`문쪽 상단 도어 스트랩(웹 래싱) · 높이 ${(z/1000).toFixed(1)}m · 측면·바닥 고정점`};if(!airbags.some(a=>Math.min(a.x+a.l,strap.x+strap.l)-Math.max(a.x,strap.x)>1&&Math.min(a.z+a.h,strap.z+strap.h)-Math.max(a.z,strap.z)>1))dunnage.push(strap)}}
  return{dunnage,airbags,reviews,transportMode,ctu:LoadwiseInsights.securing(load,{mode:transportMode})};
}
